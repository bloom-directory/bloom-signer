    ...
    async fn dispatch_inner(
        ...
        match request {
            ...
            Request::CeremonyPrepare(request) => Ok(Response::CeremonyPrepare(match request {
                ...
                SignerCeremonyPrepareRequest::OwnerAttestationPrepare(request) => {
                    SignerCeremonyPrepareResponse::OwnerAttestationPrepare(
                        self.ceremony.prepare_owner_attestation(*request, now_ms)?,
                    )
                }
            })),
            Request::CeremonyComplete(request) => Ok(Response::CeremonyComplete(match request {
                ...
                SignerCeremonyCompleteRequest::OwnerAttestationComplete(request) => {
                    SignerCeremonyCompleteResponse::OwnerAttestationComplete(Box::new(
                        self.ceremony.complete_owner_attestation(*request, now_ms)?,
                    ))
                }
            })),
...
mod tests {
    ...
    async fn policy_ceremony_and_signing_paths_do_not_emit_marker_secrets() {
        ...
        let (payloads, hashes) = match &ceremony_terms.selector {
            ...
            ApprovalSelector::Delegated { .. } => unreachable!("fixture is exact"),
        };
        service
            .ceremony
            .prepare_approval(
        ...
        let custody_marker = "marker-custody-wallet-secret-do-not-log";
        service
            .ceremony
            .prepare_custody(
                CustodyPrepareRequest {
                    ...
                    key_ref: None,
                    exact_terms_digest: Digest32::from_bytes([95; 32]),
